...
class videoHandler:
    ...
    def split_video(self, filename, timestamps):
        for i in range(len(timestamps)):
            start_time = timestamps[i]
            output_filename = f"{filename}_{i}.mp4"
    ...
    def split_videos(self, input_files, timestamps_list):
        for i, filename in enumerate(input_files):
            if filename:
                self.logger.log(LogLevel.DEBUG, f"正在处理第{i+1}个视频文件{filename}")
                self.split_video(filename, timestamps_list[i])
